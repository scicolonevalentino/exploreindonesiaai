#!/usr/bin/env python3
"""Build + validate the 2-day Kawah Ijen blue fire draft article, write JSON snapshot.

Mini-DSL for body text (see para()):
  **bold**                       -> strong decorator
  [label](AFF:PLACEHOLDER_ID)    -> inline affiliateLinkRef (must be declared in AFFILIATES)
  [label](/trips/slug)           -> inline externalLink (in-app link)
"""
import json, os, re, sys

SLUG = "2-days-kawah-ijen-blue-fire"
ID = f"itinerary-{SLUG}"

# ---- deterministic key generator ----
_n = {"v": 0}
def k():
    _n["v"] += 1
    return f"k{_n['v']:04d}"

# ---- affiliate registry (all links verified live / standard partner deep-links) ----
AFFILIATES = [
    {
        "_key": k(), "_type": "affiliateLink",
        "placeholderId": "AIRALO_INDONESIA",
        "partner": "airalo",
        "anchorText": "an Indonesia eSIM with Airalo",
        "affiliateUrl": "https://airalo.tpx.lu/ywUnFTCe",
        "publicUrl": "https://www.airalo.com/indonesia-esim",
        "experienceCategory": "esim_connectivity",
        "linkStatus": "live", "notes": "travelpayouts",
    },
    {
        "_key": k(), "_type": "affiliateLink",
        "placeholderId": "12GO_BALI_BANYUWANGI",
        "partner": "12go",
        "anchorText": "compare boats, trains and buses to Banyuwangi on 12Go",
        "affiliateUrl": "https://12go.asia/?z=16022946",
        "publicUrl": "https://12go.asia",
        "experienceCategory": "transport",
        "linkStatus": "live", "notes": None,
    },
    {
        "_key": k(), "_type": "affiliateLink",
        "placeholderId": "VIATOR_IJEN_BLUE_FIRE",
        "partner": "viator",
        "anchorText": "a guided Kawah Ijen blue fire night hike from Banyuwangi",
        "affiliateUrl": "https://www.viator.com/tours/Banyuwangi/Ijen-Blue-Fire-Tours/d50506-27057P1?pid=P00303362&mcid=42383&medium=link&campaign=KawahIjen2DayBlueFire",
        "publicUrl": "https://www.viator.com/tours/Banyuwangi/Ijen-Blue-Fire-Tours/d50506-27057P1",
        "experienceCategory": "volcano_sunrise",
        "linkStatus": "live", "notes": None,
    },
    {
        "_key": k(), "_type": "affiliateLink",
        "placeholderId": "VIATOR_IJEN_FROM_BALI",
        "partner": "viator",
        "anchorText": "a round-trip Ijen midnight tour from Bali with hotel drop-off",
        "affiliateUrl": "https://www.viator.com/tours/Banyuwangi/Kawah-Ijen-Midnight-Tour-1-Day/d50506-178080P3?pid=P00303362&mcid=42383&medium=link&campaign=KawahIjen2DayFromBali",
        "publicUrl": "https://www.viator.com/tours/Banyuwangi/Kawah-Ijen-Midnight-Tour-1-Day/d50506-178080P3",
        "experienceCategory": "volcano_sunrise",
        "linkStatus": "live", "notes": None,
    },
]
DECLARED = {a["placeholderId"] for a in AFFILIATES}

# ---- inline text parser: returns (children[], markDefs[]) ----
TOKEN = re.compile(r"\[([^\]]+)\]\(([^)]+)\)|(\*\*[^*]+\*\*)")

def para(text, style="normal"):
    children, markDefs = [], []
    pos = 0
    for m in TOKEN.finditer(text):
        if m.start() > pos:
            children.append({"_key": k(), "_type": "span", "marks": [], "text": text[pos:m.start()]})
        if m.group(3):  # **bold**
            children.append({"_key": k(), "_type": "span", "marks": ["strong"], "text": m.group(3)[2:-2]})
        else:
            label, href = m.group(1), m.group(2)
            mk = k()
            if href.startswith("AFF:"):
                pid = href[4:]
                if pid not in DECLARED:
                    raise ValueError(f"undeclared affiliate placeholder: {pid}")
                markDefs.append({"_key": mk, "_type": "affiliateLinkRef", "placeholderId": pid})
            else:
                markDefs.append({"_key": mk, "_type": "externalLink", "href": href})
            children.append({"_key": k(), "_type": "span", "marks": [mk], "text": label})
        pos = m.end()
    if pos < len(text):
        children.append({"_key": k(), "_type": "span", "marks": [], "text": text[pos:]})
    if not children:
        children = [{"_key": k(), "_type": "span", "marks": [], "text": text}]
    return {"_key": k(), "_type": "block", "style": style, "markDefs": markDefs, "children": children}

def h2(text):
    return {"_key": k(), "_type": "block", "style": "h2", "markDefs": [],
            "children": [{"_key": k(), "_type": "span", "marks": [], "text": text}]}

def ul(items):
    # PortableText list items are blocks with listItem
    out = []
    for it in items:
        b = para(it)
        b["listItem"] = "bullet"
        b["level"] = 1
        out.append(b)
    return out

# ---- body ----
body = []
body += [
    h2("Who this trip is for"),
    para("This is a short, focused trip for couples, friends and solo travellers who want one unforgettable volcano night without committing to a full Java loop. You give up a single night of sleep to stand on the rim of Kawah Ijen for the electric-blue flames, watch the sun come up over the world's largest acidic crater lake, and be back at your hotel by mid-morning. It works as a standalone two-day escape from Banyuwangi, and it is the natural bolt-on to a Bali holiday, since the ferry crossing is short."),
    para("It is not the right plan if your priority is rest, or if you are travelling with very young children. The trek starts around midnight, the air at the rim is cold, and the crater floor holds toxic sulphur gas that needs a mask. If that sounds like too much, skip it and enjoy Bali or Lombok instead. Done with open eyes, though, it is one of the most striking few hours in Indonesia."),
    h2("Trip at a glance"),
    para("Two days, one night, one base in the Banyuwangi area on the eastern tip of Java. Arrive on day one, sort your gear and sleep early. Around half past midnight you set off for the roughly 1.5 hour climb to the crater rim, drop into the crater for the blue fire, then come back up to watch sunrise turn the lake turquoise. You are usually back down by late morning on day two, with the afternoon free to rest or travel onward."),
    para("**Travel note:** Sort your data before you arrive. Activating [an Indonesia eSIM with Airalo](AFF:AIRALO_INDONESIA) before you cross to Java means maps and your guide's WhatsApp work from the moment you land, which matters when you are confirming a small-hours pickup the night before."),
    h2("How to get to Banyuwangi"),
    para("Kawah Ijen sits above Banyuwangi, the port town on the far eastern tip of Java, directly across the strait from Bali. The most common approach is from Bali: drive or ride to Gilimanuk in the far west, then take the Ketapang ferry, which runs around the clock and crosses in under an hour. Remember Bali (WITA) runs one hour ahead of Java (WIB), so reset your watch on the boat. From further afield, Banyuwangi has its own airport (code BWX) and a train station with services from Surabaya and Probolinggo."),
    para("Because the pieces vary by season and day, it is worth checking live options in one place. You can [compare boats, trains and buses to Banyuwangi on 12Go](AFF:12GO_BALI_BANYUWANGI) to line up the ferry and any onward train before you go. If you would rather not organise the crossing yourself, many travellers book [a round-trip Ijen midnight tour from Bali with hotel drop-off](AFF:VIATOR_IJEN_FROM_BALI), which folds the long transfer, the ferry and the trek into one overnight package."),
    h2("Day 1: Arrive, prepare and sleep early"),
    para("Aim to reach your Banyuwangi accommodation by late afternoon. This is a logistics day, not a sightseeing one, and the single most useful thing you can do is rest. Confirm your pickup time with your guide, lay out warm layers and a head torch, and eat a proper dinner early. Local warungs around town do good, cheap Javanese food, and rice with grilled fish or chicken is the right kind of fuel for a cold night hike."),
    para("Pack a small daypack with water, snacks, gloves, a hat and a buff or scarf you can pull over your face if the wind turns. Temperatures at the rim can fall to around 5 to 10 degrees Celsius before dawn, which surprises people arriving from the Bali heat. Then get to bed by eight if you can, because the alarm is brutal."),
    h2("Day 2: The blue fire, the sunrise and the crater lake"),
    para("Pickup is usually around half past midnight. From the Paltuding base you start the walk to the crater rim, a steady climb of roughly two to three kilometres that most people cover in about 1.5 hours. The path is wide but relentlessly uphill, and porters with two-wheeled carts pass in the dark, so keep to the side. Booking [a guided Kawah Ijen blue fire night hike from Banyuwangi](AFF:VIATOR_IJEN_BLUE_FIRE) is the simplest way to do this well, since the price bundles the pickup, entrance fees, a guide who knows the timing, and the gas mask you genuinely need."),
    para("At the rim you make the steep descent into the crater itself to see the blue fire up close, an eerie electric-blue flame where sulphuric gases ignite as they escape the vents. This part is optional and demanding: the trail down is loose and about 45 minutes each way, and the sulphur clouds can be thick, which is exactly why the mask matters. If the gas is heavy or you would rather not scramble, the flames are still visible from the rim."),
    para("As the sky lightens you climb back to the rim and the reward changes character entirely. The dawn light reveals the turquoise crater lake below, its colour coming from the extreme acidity of the water, while the miners haul baskets of bright yellow sulphur up the same path you just walked. Take your photos, then start the walk down. Most groups are back at Paltuding by mid-morning and at their hotel not long after, which leaves the rest of day two for sleep, a slow lunch, or the onward journey."),
    h2("What the trek is actually like"),
    para("Be honest with yourself about the effort. This is a night hike on an active volcano, not a viewpoint you drive to. The climb to the rim is the fitness test; the descent into the crater is the nerve test. Wear proper closed shoes with grip, bring a head torch so your hands stay free, and do not rely on your phone for light. The sulphur smell reaches the rim on a bad wind, so a mask or at least a damp buff is worth having even if you skip the crater floor."),
    para("Go at your own pace and let faster hikers pass. There is no prize for reaching the top first, and altitude plus cold plus a 1am start makes everyone slower than they expect."),
    h2("Costs, permits and the first-Friday closure"),
    para("Entrance for foreign visitors is modest, in the region of IDR 100,000 on weekdays and IDR 150,000 at weekends, as a working estimate; a guided tour usually rolls this into the price. One thing that catches people out: Ijen closes on the first Friday of every month for the Ijen Resik cleaning programme, so do not schedule your trek for that night. Conditions on the mountain can also close the crater floor at short notice when gas levels are high, and a good guide will make that call for you."),
    para("A gas mask, ideally a proper respirator rather than a paper one, is the piece of gear worth insisting on. If your tour hands you a flimsy dust mask, ask for better before you start down."),
    h2("Where to stay"),
    para("Most travellers base themselves in or near Banyuwangi town, where guesthouses and small hotels are inexpensive and used to guests leaving in the middle of the night. Staying in town keeps you close to the ferry, the station and the airport for an easy onward move. If you would rather wake up closer to the trailhead, there are simpler homestays and lodges up towards Licin on the road to the Paltuding base, which shaves time off the pre-dawn drive but offers fewer places to eat."),
    para("Whichever you choose, tell your accommodation you are doing the Ijen trek. Many will hold your room late, arrange the guide, and have coffee ready when you stagger back down."),
    h2("Adding Kawah Ijen to a longer trip"),
    para("Two days at Ijen slots neatly onto the end of a Bali holiday, or onto a Java route. If you are pairing it with Bali, our [5 Days in Bali: Ubud, Canggu and Uluwatu](/trips/5-days-bali-ubud-canggu-uluwatu) and [7 Days in Bali for First-Timers](/trips/7-days-bali-first-timers) plans both end within easy reach of the ferry. Coming the other way across Java, this trip is the missing eastern piece of our [5 Days in Yogyakarta and Mount Bromo](/trips/5-days-yogyakarta-bromo) route, and both volcanoes together are covered in [7 Days in Yogyakarta and East Java](/trips/7-days-yogyakarta-east-java) and the longer [15 Days in Java and Bali](/trips/15-days-java-bali)."),
    h2("Frequently asked questions"),
]

FAQ = [
    ("Is two days enough for Kawah Ijen?",
     "Yes. Kawah Ijen is a single night-time experience, not a multi-day one, so two days and one night is the honest right length: a day to reach Banyuwangi and rest, then the midnight trek, the blue fire and the sunrise before you head back or move on. Adding a second night only makes sense if you also want to slow down in Banyuwangi or add a nearby waterfall."),
    ("What time does the blue fire trek start?",
     "Pickup is usually around half past midnight, with the climb to the rim taking about 1.5 hours, because the electric-blue flames are only visible in full darkness. You want to be down in the crater in the small hours and back at the rim for sunrise, so a 1am start on the mountain is normal."),
    ("How hard is the hike to the crater?",
     "The climb to the rim is a steady uphill walk of roughly two to three kilometres over about 1.5 hours, manageable for anyone reasonably fit. The optional descent into the crater to see the blue fire up close is harder: it is steep, loose underfoot, around 45 minutes each way, and passes through sulphur gas, which is why a proper mask and a guide matter."),
    ("Do I need a gas mask at Kawah Ijen?",
     "Yes, especially if you go down to the crater floor for the blue fire. The sulphuric gas is genuinely toxic in heavy gusts, so a proper respirator-style mask is essential and reaches the rim on a bad wind too. Guided tours normally provide one; if yours offers only a thin paper mask, ask for a better one before you descend."),
    ("Can I do Kawah Ijen as a day trip from Bali?",
     "You can, and many people do it as one long overnight loop: cross on the Ketapang ferry, trek through the night and return to Bali the next day. It is efficient but exhausting. Basing yourself in Banyuwangi for a night before and, ideally, after the trek turns a punishing round trip into a far more comfortable two-day escape."),
    ("When is the best time to visit Kawah Ijen?",
     "The dry season from April to October gives the clearest skies and the safest trail, and is the most reliable window for both the blue fire and the sunrise. Avoid the first Friday of any month, when the mountain closes for the Ijen Resik cleaning programme, and check the forecast, since heavy rain or high gas levels can close the crater floor."),
]
faq = [{"_key": k(), "_type": "faqItem", "question": q, "answer": a} for q, a in FAQ]

# ---- document ----
doc = {
    "_id": ID,
    "_type": "article",
    "title": "2 Days at Kawah Ijen: Blue Fire, Sunrise and the Crater Lake",
    "slug": {"_type": "slug", "current": SLUG},
    "contentStatus": "draft",
    "destinationPrimary": "java",
    "travelStylePrimary": "adventure_volcanoes",
    "tripLengthBucket": "short_escape",
    "tripLengthDays": 2,
    "travellerTypes": ["couples", "friends", "solo_travellers"],
    "vibe": "active",
    "experienceTags": ["volcano_sunrise", "overland_route"],
    "bestSeason": "April–October",
    "route": "Bali/Surabaya → Banyuwangi → Kawah Ijen → Banyuwangi",
    "focusKeyword": "Kawah Ijen itinerary",
    "metaTitle": "2 Days at Kawah Ijen: Blue Fire and Crater Lake",
    "metaDescription": "A focused 2-day, 1-night Kawah Ijen plan from Banyuwangi: the midnight blue fire trek, sunrise over the acid lake, costs, safety and how to add it to Bali.",
    "intro": "Two days is exactly enough for Kawah Ijen if you treat it as one focused volcano night rather than a slow holiday. You reach Banyuwangi, sleep early, trek at midnight for the electric-blue flames, watch sunrise fill the turquoise crater lake, and are back by late morning. Here is the honest version, with the cold, the sulphur and the logistics included.",
    "author": {"_ref": "author-editorial-team", "_type": "reference"},
    "affiliateLinks": AFFILIATES,
    "body": body,
    "faq": faq,
}

# ---- validation ----
errors = []
keys = []
def collect(o):
    if isinstance(o, dict):
        if "_key" in o: keys.append(o["_key"])
        for v in o.values(): collect(v)
    elif isinstance(o, list):
        for v in o: collect(v)
collect(doc)
if len(keys) != len(set(keys)):
    errors.append("duplicate _key values")

refs = set()
def find_refs(o):
    if isinstance(o, dict):
        if o.get("_type") == "affiliateLinkRef": refs.add(o["placeholderId"])
        for v in o.values(): find_refs(v)
    elif isinstance(o, list):
        for v in o: find_refs(v)
find_refs(body)
missing = refs - DECLARED
if missing: errors.append(f"affiliateLinkRef without affiliateLinks entry: {missing}")
unused = DECLARED - refs
if unused: errors.append(f"declared affiliate never used inline: {unused}")

day_h2 = [b for b in body if b.get("style") == "h2" and b["children"][0]["text"].startswith("Day")]
bad_days = [b["children"][0]["text"] for b in day_h2 if not re.match(r"^Day \d+:", b["children"][0]["text"])]
if bad_days: errors.append(f"day headings not 'Day N:': {bad_days}")

# house rules
full_text = json.dumps(body, ensure_ascii=False)
if "—" in full_text: errors.append("em-dash present (use commas)")
if len(doc["metaTitle"]) > 60: errors.append(f"metaTitle too long: {len(doc['metaTitle'])}")
if len(doc["metaDescription"]) > 160: errors.append(f"metaDescription too long: {len(doc['metaDescription'])}")

print("=== VALIDATION ===")
print(f"blocks: {len(body)}  faq: {len(faq)}  affiliates: {len(AFFILIATES)}")
print(f"affiliateLinkRefs used: {sorted(refs)}")
print(f"h2: {[b['children'][0]['text'] for b in body if b['style']=='h2']}")
print(f"metaTitle len: {len(doc['metaTitle'])}  metaDescription len: {len(doc['metaDescription'])}")
print(f"total keys: {len(keys)}  unique: {len(set(keys))}")
if errors:
    print("FAILURES:"); [print("  -", e) for e in errors]; sys.exit(1)
print("ALL CHECKS PASSED")

out = f"/Users/valentinoscicolone/Desktop/ExploreIndonesiaAI/content/articles/{SLUG}.json"
os.makedirs(os.path.dirname(out), exist_ok=True)
with open(out, "w") as f:
    json.dump(doc, f, ensure_ascii=False, indent=2)
print(f"snapshot written: {out}")
